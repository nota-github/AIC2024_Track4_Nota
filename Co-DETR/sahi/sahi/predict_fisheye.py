...
def get_sliced_prediction(
    ...
    # additional
    postprocess2_constructor = POSTPROCESS_NAME_TO_CLASS["NMS"]
    postprocess2 = postprocess2_constructor(
        match_threshold=0.8,
        match_metric="IOS",
    ...
    for group_ind in range(num_group):
        ...
        for object_prediction in prediction_result.object_prediction_list:
            if object_prediction:  # if not empty
                object_prediction = object_prediction.get_shifted_object_prediction()
                w, h, _ = image_list[0].shape
                h_sh, w_sh = shift_amount_list[0]
                border_h = h if h_sh == 0 else h_sh
                border_w = w if w_sh == 0 else w_sh
                x1,y1,x2,y2 = object_prediction.bbox.to_xyxy()
                if abs(x1-border_h) < 5 or abs(x2-border_h) < 5 or abs(y1-border_w) < 5 or abs(y2-border_w) < 5:
                    object_prediction.score.value = 0.5
    ...
    if len(object_prediction_list) > 1:
        object_prediction_list = postprocess(object_prediction_list)
        # additianal
        object_prediction_list = postprocess2(object_prediction_list)
    time_end = time.time() - time_start
    durations_in_seconds["prediction"] = time_end
